...
const {outputDir, packagesDir, pkgTemplate, npmAccess, npmScope, versionPlaceholder} = require("../../repo.config");
  ...
  publish() {
    findPackages().map(pkgName => {
      ...
      try {
        ...
        sync("npm", ["publish", "--userconfig", npmrc, "--access", npmAccess, "--registry", registry], {
          cwd,
