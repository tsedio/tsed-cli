    ...
    return [
      {
        ...
        task: () => this.renderService.render("generic.spec.hbs", data, `${outputFile}.spec.ts`)
      },
      {
        title: `Generate ${options.type} integration file '${outputFile}.integration.spec.ts'`,
        enabled() {
          return options.type === "controller";
        },
        task: () => this.renderService.render("generic.integration.hbs", data, `${outputFile}.integration.spec.ts`)
      }
    ];
  }
